...
start_a	local
	...
	movlf	0xff,0x1f
	clrf	PORTA ;The contents of register PORTA are cleared and the Z bit is set
	clrf	PORTB ;The contents of register PORTB are cleared and the Z bit is set
	movlw	b'11111111'
	tris	PORTA
	tris	PORTB
	endlocal
...
xyout	bcf	STATUS,C
	...
	iorlw	b'00010000'
        ...
        if      BOOSTRMB ;;
	andlw	b'00111111'
	btfsc	INDF,3 ;;RMB, Skip if bit 3 of INDF is clear
	endif	;;next command will be skipped, if compiled with BOOSTRMB and TRISB3=0 (RMB presssssscious)

	iorlw	b'11000000' ; Bitwise OR, set bits 11 to RA7/6
    
    movwf	q0 ;move W(where PORTA is next located) to xtemp
    movwf	q1 ;move W(where PORTA is next located) to ytemp
    ; andlw   b'11111001'
    bcf     q1,1 ; set PORTA1 to 0
    bcf     q1,2 ; set PORTA2 to 0
    btfsc   q0,1 ; check PORTA1 is 1, skip if 0
    bsf     q1,2 ; set PORTA2 to 1
    btfsc   q0,2 ; check PORTA2 is set, skip if clear
    bsf     q1,1 ; set PORTA1 to 1
    
    ; result in ytemp
    movfw   q1
    
	btfsc	flags,fon ;Skip if bit fon of flags is clear, if device is off - skip
	tris	PORTA ; Copy W to PORTA - Amiga/Atari register
